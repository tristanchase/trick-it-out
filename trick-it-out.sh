...
function __google_chrome__ {
	set -o errexit
	_trickdir="${HOME}/Downloads/trick-dir"
	mkdir -p "${_trickdir}"
	...
	if [[ "${_yN}" =~ (y|Y) ]]; then
		cd "${_trickdir}"
		wget https://raw.githubusercontent.com/tristanchase/trick-it-out/main/install-helpers/zzz.sh
		wget https://raw.githubusercontent.com/tristanchase/trick-it-out/main/install-helpers/zzz-google-chrome-upgrade
		mv zzz.sh zzz
		chmod 755 *
